...
const VehicleForm: React.FC = () => {
  ...
  const ESIM_API_URL = 'https://api.esim-go.com/v2.5/esims';
  const ESIM_ICCID = '8940102506200241067'; // ICCID furnizat
  const ESIM_API_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI2OGI0NTcwY2Y4MjkzYzRlM2Y2NmYzNWYifQ.4Jlpsb-Ure4i-x4y4nw8cff9p1A6LLMuJfIuIJg89N8'; // API Key
  ...
  // Send SMS notification via eSIM-GO API
  const sendSMSNotification = async (vehicle: Vehicle) => {
    try {
      ...
      setMessage(`📱 Se trimite SMS către ${phoneNumber} pentru ${nume} prin eSIM-GO...`);
      
      // Mesajul personalizat pentru vehicul (max 160 caractere)
      const personalizedMessage = `Buna ziua ${nume}! ITP-ul vehiculului ${nrInmatriculare} expira in ${diffDays} ${diffDays === 1 ? 'zi' : 'zile'}. Va rugam sa-l reinnnoiti!`;
      
      // Verifică lungimea mesajului
      if (personalizedMessage.length > 160) {
        setMessage('❌ Mesajul este prea lung (>160 caractere)');
        return;
      }

      // Pentru testare - logurile pentru debugging
      console.log('eSIM-GO SMS Request:', {
        url: `${ESIM_API_URL}/${ESIM_ICCID}/sms`,
        to: phoneNumber,
        nume: nume,
        vehicul: nrInmatriculare,
        message: personalizedMessage,
        messageLength: personalizedMessage.length,
        apiKey: ESIM_API_KEY.substring(0, 20) + '...'
      });

      // Request către eSIM-GO API
      const response = await fetch(`${ESIM_API_URL}/${ESIM_ICCID}/sms`, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          message: personalizedMessage,
          from: 'ITP Alert' // Sender name
        })
      });

      const result = await response.json();
      console.log('eSIM-GO Response:', result);

      if (response.ok && result.status === 'success') {
        setMessage(`✅ SMS trimis cu succes către ${nume} (${phoneNumber}) prin eSIM-GO! Status: ${result.status}`);
        console.log('SMS trimis cu succes prin eSIM-GO');
      } else {
        const errorMsg = result.message || result.error || response.statusText || 'Eroare necunoscută';
        setMessage(`❌ Eroare eSIM-GO API: ${errorMsg}`);
        console.error('eSIM-GO API Error:', result);
      }

    } catch (error) {
      console.error('Eroare SMS eSIM-GO:', error);
      setMessage('❌ Eroare la trimiterea SMS prin eSIM-GO - verifică consola pentru detalii');
    } finally {
      setLoading(false);
    }
  };
  ...
  const sendSMSToAllExpiring = async () => {
    if (!confirm('Sigur vrei să trimiți SMS către toate vehiculele cu ITP care expiră în 7 zile prin eSIM-GO?')) {
      return;
    }
    ...
    setMessage(`📱 Se trimit ${expiringVehicles.length} SMS-uri prin eSIM-GO...`);
    ...
    setMessage(`✅ Toate SMS-urile au fost procesate pentru ${expiringVehicles.length} vehicule prin eSIM-GO!`);
  };
          ...
          <div className="card-header bg-primary text-white text-center d-flex justify-content-between align-items-center">
            ...
            {vehicles.filter(v => isExpiringSoon(v.valabilitate)).length > 0 && (
              <button
                ...
                disabled={loading}
                title={`Trimite SMS către toate vehiculele care expiră prin eSIM-GO (${vehicles.filter(v => isExpiringSoon(v.valabilitate)).length})`}
              >
                📱 eSIM SMS la toate ({vehicles.filter(v => isExpiringSoon(v.valabilitate)).length})
              </button>
            )}
          </div>
                          ...
                          <div className="btn-group" role="group">
                            ...
                            {expiring && (
                              <button
                                ...
                                title={`Trimite SMS prin eSIM-GO către ${vehicle.nume} - ${vehicle.nr_telefon}`}
                                disabled={loading}
                              >
                                📱 eSIM SMS
                              </button>
